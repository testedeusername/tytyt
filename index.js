...
**📦 | Estoque: ** __${db.get(`${interaction.customId}.conta`).length}__`)
        ...
        if (quantidade < 1) {
          const embedsemstock = new Discord.MessageEmbed()
            ...
            .setDescription(`☹️ | Estamos sem estoque no momento, volte mais tarde!`)
            .setColor(config.get(`color`))
        ...
        interaction.guild.channels.create(`🛒・carrinho-${interaction.user.username}`, {
          ...
          .addComponents(
           ...
           const embedss = new Discord.MessageEmbed()
             ...
             .addField(`⭐️ | ID da compra:`, `${data_id}`) 
             .setColor(config.get(`color`))
             .setThumbnail(client.user.displayAvatarURL())
             ...
             collector.on("collect", intera => {
               ...
               if (intera.customId === 'cancelarbuy') {
                 ...
                 const embedcancelar = new Discord.MessageEmbed()
                            ...
                            .setDescription(`:x: | Você cancelou a compra e todos os produtos foram devolvido para o estoque. Você pode voltar a comprar quando quiser!`)
                            .setColor(config.get(`color`))
                            .setThumbnail(client.user.displayAvatarURL())
                                  ...
                                  const vendadel = setTimeout(function () {
                                   ...
                                   const canalif1 = client.channels.cache.get(config.canallogs);
                                     ...
                                     if (a < quantidade1) {
                                       ...
                                       msg.channel.send(`⭐️ | ID da compra: ${data_id}`)
                                           ...
                                           const removed = a.splice(0, Number(quantidade1));
                                            db.set(`${severi}.conta`, a);
                                             const embedentrega = new Discord.MessageEmbed()
                                               ...
                                               .setDescription(`**🎁 | Seu(s) produto(s) chegou!** \n  \`\`\`${removed.join("\n")}\`\`\`\n**❤️ | Obrigado pela compra, volte sempre!**\n\n⭐️ | **ID da compra:** ${data_id}`)
                                               .setColor(config.get(`color`))
                                               //.setThumbnail(client.user.displayAvatarURL())
                                               .setImage(`${config.get(`banner`)}`)
                                               ...
                                               if (interaction.isButton()) {
                                                 ...
                                                 for (let i = 0; i != estrelas; i++) {
                                                   ...
                                                   embed.reply("❤️ | Obrigado pela avaliação!").then(msg => {
                                                     ...
                                                     .addField(`:identification_card: | Comprador(a):`, `<@${interaction.user.id}>`)
                                                     .addField(`📅 | Data da compra:`, `${moment().format('LLLL')}`)
...
\`\`\``)
  ...
  client.on("messageCreate", message => {
    ...
    .setColor("BLACK")
    .setDescription(`**🧨 | Olá <@${message.author.id}>, sou um Bot com um sistema de vendas automáticas e o meu prefixo é \` ${config.get(`prefix`)} \` \n\n🤖| Veja minha lista de comandos utilizando o comando \`${config.get(`prefix`)}ajuda\` \n\n🤝 | Caso queira adquirir um Bot igual a este, entra na loja [clicando aqui](https://discord.gg/xaazfHsB)**`)
